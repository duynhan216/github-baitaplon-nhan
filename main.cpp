...
int main(int argc, char* argv[])
{
    ...
    while(!is_quit)
    {
        ...
        for(auto it = hpList.begin(); it != hpList.end();)
        {
            ...
            if (hp != NULL)
            {
                ...
                if (CheckCollision(rect_player, rect_hp))
                {
                    ...
                    delete hp;
                    it = hpList.erase(it);
                    std::cout << p_player.get_health() << std::endl;
                    continue;
                }
            }
            ++it;
        ...
        hp_text.LoadFromRenderText(font, g_screen);
        hp_text.RenderText(g_screen, 10, 10);

        coins_text.setText("Coins: " + std::to_string(coinCounter));
        coins_text.LoadFromRenderText(font, g_screen);
        coins_text.RenderText(g_screen, 10, 40);

        score_text.setText("Score: " + std::to_string(p_player.GetScore()));
        score_text.LoadFromRenderText(font, g_screen);
        score_text.RenderText(g_screen, 10, 70);
        ...
        shieldDur_text.LoadFromRenderText(font, g_screen);
        shieldDur_text.RenderText(g_screen, 700, 10);

        Cooldown_text.setText("COOLDOWN :");
        Cooldown_text.LoadFromRenderText(font, g_screen);
        Cooldown_text.RenderText(g_screen, 300, 50);

        shieldCooldown_text.setText(shield_cd_str);
        shieldCooldown_text.LoadFromRenderText(font, g_screen);
        shieldCooldown_text.RenderText(g_screen, 700, 90);

        bulletCooldown_text.setText(bullet_str);
        bulletCooldown_text.LoadFromRenderText(font, g_screen);
        bulletCooldown_text.RenderText(g_screen, 700, 130);
